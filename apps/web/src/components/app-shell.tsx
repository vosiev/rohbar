...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void api.auth.me().then(result => {
      ...
      setUser(result.data);
      setStoredRole(result.data.role);
      if (!canAccess(path, result.data.role)) router.replace("/dashboard");
      else setReady(true);
    });
    return () => { active = false; };
  }, [path, router]);
