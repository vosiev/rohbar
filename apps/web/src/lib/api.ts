...
const mockOffers:ShipmentOffer[]=[{id:"OF-291",shipmentId:"RH-10482",carrierId:"car_001",carrierName:"ООО «СтройТранс»",vehicle:"Тент 20 т · А123ВС",price:85000,currency:"RUB",etaDays:2,status:"pending",createdAt:"2026-09-08T08:40:00Z"},{id:"OF-288",shipmentId:"RH-10482",carrierId:"car_002",carrierName:"ООО «СеверЛогистик»",vehicle:"Тент 20 т · М456КМ",price:89000,currency:"RUB",etaDays:2,status:"pending",createdAt:"2026-09-08T08:31:00Z"},{id:"OF-284",shipmentId:"RH-10482",carrierId:"car_003",carrierName:"ИП Абдуллоев",vehicle:"Фура 20 т · Т789ОР",price:92000,currency:"RUB",etaDays:3,status:"pending",createdAt:"2026-09-08T08:20:00Z"}];
const mockFleet:FleetVehicle[]=[{id:"VH-001",plate:"А123ВС",model:"КАМАЗ 54901",type:"Грузовик",body:"Тент",capacityTons:20,status:"available"},{id:"VH-002",plate:"М456КМ",model:"MAN TGX",type:"Седельный тягач",body:"Тент",capacityTons:20,status:"on_route"}];
...
const mockOr=(mock:()=>unknown,path:string,init?:RequestInit)=>mode==="mock"?Promise.resolve({data:mock(),error:null} as never):request(path,init);
...
export const api={
 ...
 shipments:{
  ...
  accept:async(id:string)=>mode==="mock"?{data:mockShipments.find(x=>x.id===id)||mockShipments[0],error:null}:request<Shipment>(`/api/v1/shipments/${id}/accept`,{method:"POST"}),
 ...
 fleet:{
  ...
  create:async(payload:unknown)=>mode==="mock"?{data:{...mockFleet[0],...payload,id:`VH-${Date.now().toString().slice(-3)}`},error:null}:request<FleetVehicle>("/api/v1/fleet",{method:"POST",body:JSON.stringify(payload)})
 },
 drivers:{
  assign:async(shipmentId:string,payload:unknown)=>mode==="mock"?{data:{shipmentId,...payload} as DriverAssignment,error:null}:request<DriverAssignment>(`/api/v1/shipments/${shipmentId}/driver`,{method:"POST",body:JSON.stringify(payload)})
 },
 notifications:{
